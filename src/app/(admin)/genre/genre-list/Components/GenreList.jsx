...
const GenreCard = ({ idx, data, setIsUpdate, toggle, setCurrentData, handleStatus, currentPage, handleDelete }) => {
  return (
    <tr>
...
const GenreList = () => {
  ...
  const { data: genreData, isLoading } = useGetGenreQuery({ page: currentPage, search: debounceSearch, disable: status })
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this genre?')) {
      try {
        const res = await deleteGenre(id).unwrap()
        if (res?.success) {
        ...
        console.log(error, 'Genre delete error')
  ...
  const handlePageClick = (page) => {
    if (page >= 1 && page <= genreData?.pagination?.totalPages) {
      setCurrentPage(page)
    }
  }
  ...
  const renderPagination = () => {
    const pages = []
    for (let i = 1; i <= genreData?.pagination?.totalPages; i++) {
      pages.push(
    ...
    <>
      {isLoading || !genreData ? (
        ...
        <Card>
          ...
          <CardFooter className="border-top">
            <nav aria-label="Page navigation">
              <ul className="pagination justify-content-end mb-0">
                ...
                <li className={`page-item ${currentPage === genreData?.pagination?.totalPages ? 'disabled' : ''}`}>
                  <button
                    className="page-link"
                    onClick={() => handlePageClick(currentPage + 1)}
                    disabled={currentPage === genreData?.pagination?.totalPages}>
                    Next
                  </button>
                </li>
              </ul>
          ...
          <GenreForm isUpdate={isUpdate} isTrue={isTrue} toggle={toggle} currentData={currentData} />
        </Card>
      )}
    </>
  )
